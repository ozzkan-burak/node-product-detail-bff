...
app.get('/api/products/:id', async (req, res) => {
  try{
    const [productResponse, stockResponse, reviewResponse] = await Promise.all([
      axios.get(`http://localhost:${PRODUCT_SERVICE_PORT}/products/${req.params.id}`),
      axios.get(`http://localhost:${STOCK_SERVICE_PORT}/stock/${req.params.id}`),
      axios.get(`http://localhost:${REVIEW_SERVICE_PORT}/reviews/${req.params.id}`)
    ]);
    ...
    console.error('Ürün detayı alınırken hata oluştu:', error.message);
      if (error.response) {
            res.status(error.response.status).json({ message: error.response.data.message || 'Bir hata oluştu.' });
        } else {
